...
class AndroidSignalIngestor(
  ...
  private fun gps(): SecurityObservation {
    ...
    return SecurityObservation(
      id = "gps-${location.time}", kind = ObservationKind.GPS, observedAtEpochMs = location.time,
      source = EvidenceSource.LOCAL_ANDROID,
      payload = buildMap {
        put("latitude", location.latitude.toString()); put("longitude", location.longitude.toString())
        put("accuracy_m", location.accuracy.toString()); put("provider", location.provider.orEmpty())
      },
    )
  }
  ...
  private fun cellular(): List<SecurityObservation> {
    ...
    return cells.mapIndexed { index, cell ->
      SecurityObservation(
        id = "cell-${cell.hashCode()}-$index", kind = ObservationKind.CELLULAR,
        observedAtEpochMs = clock.nowEpochMs(), source = EvidenceSource.LOCAL_ANDROID,
        payload = cellIdentityPayload(cell) + mapOf("registered" to cell.isRegistered.toString()),
      )
    }
  }
  ...
  private fun wifi(): List<SecurityObservation> {
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && ContextCompat.checkSelfPermission(context, Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED) {
      return listOf(unavailable("wifi-unavailable", ObservationKind.WIFI, "location_permission=denied"))
    }
    ...
    return results.mapIndexed { index, scan -> SecurityObservation(
      id = "wifi-${scan.BSSID}-$index", kind = ObservationKind.WIFI, observedAtEpochMs = clock.nowEpochMs(), source = EvidenceSource.LOCAL_ANDROID,
      payload = mapOf("ssid" to scan.SSID.ifBlank { "Hidden SSID" }, "bssid" to scan.BSSID, "frequency_mhz" to scan.frequency.toString(), "rssi_dbm" to scan.level.toString()),
    ) }
  }
  ...
  private fun network(): SecurityObservation {
    ...
    return SecurityObservation(
      id = "network-${clock.nowEpochMs()}", kind = ObservationKind.NETWORK, observedAtEpochMs = clock.nowEpochMs(), source = EvidenceSource.LOCAL_ANDROID,
      payload = mapOf("transport" to transport(caps), "validated" to caps.hasCapability(NetworkCapabilities.NET_CAPABILITY_VALIDATED).toString()),
    )
  }
  ...
  private fun vpn(): SecurityObservation {
    ...
    return SecurityObservation(
      id = "vpn-${clock.nowEpochMs()}", kind = ObservationKind.VPN, observedAtEpochMs = clock.nowEpochMs(), source = EvidenceSource.LOCAL_ANDROID,
  ...
  private fun unavailable(id: String, kind: ObservationKind, reason: String) = SecurityObservation(
    id = id, kind = kind, observedAtEpochMs = clock.nowEpochMs(), source = EvidenceSource.UNAVAILABLE,
  ...
  private fun cellIdentityPayload(cell: CellInfo): Map<String, String> = buildMap {
    when (cell) {
      is CellInfoLte -> {
        put("radio", "LTE"); put("mcc", cell.cellIdentity.mccString.orEmpty()); put("mnc", cell.cellIdentity.mncString.orEmpty())
        put("tac", cell.cellIdentity.tac.toString()); put("ci", cell.cellIdentity.ci.toString()); put("pci", cell.cellIdentity.pci.toString()); put("earfcn", cell.cellIdentity.earfcn.toString())
        put("rsrp_dbm", cell.cellSignalStrength.rsrp.toString()); put("rsrq_db", cell.cellSignalStrength.rsrq.toString()); put("rssnr_db", cell.cellSignalStrength.rssnr.toString())
      }
      is CellInfoNr -> if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
        ...
        put("radio", "NR"); put("mcc", identity.mccString.orEmpty()); put("mnc", identity.mncString.orEmpty()); put("tac", identity.tac.toString()); put("nci", identity.nci.toString()); put("pci", identity.pci.toString()); put("nrarfcn", identity.nrarfcn.toString()); put("ss_rsrp_dbm", signal.ssRsrp.toString()); put("ss_rsrq_db", signal.ssRsrq.toString()); put("ss_sinr_db", signal.ssSinr.toString())
      }
  ...
  @SuppressLint("MissingPermission")
  private fun BleScanResult.toObservation(now: Long) = SecurityObservation(
    id = "ble-${device.address}-$now", kind = ObservationKind.BLE, observedAtEpochMs = now, source = EvidenceSource.LOCAL_ANDROID,
    payload = mapOf("address" to device.address, "name" to device.name.orEmpty(), "rssi_dbm" to rssi.toString()),
  )
}
