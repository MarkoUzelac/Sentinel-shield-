...
object ThreatSnapshotProjector {
    ...
    snapshot.observations.map { observation ->
      RadarContact(
        ...
        label = when (observation.kind) {
          ...
          ObservationKind.WIFI -> observation.payload["ssid"].orEmpty()
  ...
  fun tacticalMap(snapshot: ThreatSnapshot): List<TacticalMapPoint> = snapshot.observations.mapNotNull { observation ->
    val lat = observation.payload["latitude"]?.toDoubleOrNull()
      ?: observation.payload["opencellid_lat"]?.toDoubleOrNull()
    val lon = observation.payload["longitude"]?.toDoubleOrNull()
    ...
    TacticalMapPoint(
      id = observation.id,
      label = when (observation.kind) {
        ObservationKind.CELLULAR -> "Cell ${observation.payload["cid"] ?: observation.payload["ci"] ?: "unknown"}"
